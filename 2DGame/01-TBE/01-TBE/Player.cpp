...
enum PlayerAnims
{
	STAND_LEFT, STAND_RIGHT, STANDLR, STANDRL, MOVE_LEFT, MOVE_RIGHT
};
...
void Player::init(const glm::ivec2 &tileMapPos, ShaderProgram &shaderProgram)
{
	...
	sprite->addKeyframe(MOVE_LEFT, glm::vec2(0.1f, 0.f));
	sprite->addKeyframe(MOVE_LEFT, glm::vec2(0.2f, 0.f));
	sprite->addKeyframe(MOVE_LEFT, glm::vec2(0.3f, 0.f));
...
void Player::update(int deltaTime)
{
	...
	{
		if (direction != "left" && sprite->animFinished()){
			sprite->changeAnimation(STANDRL);
			direction = "left";
		}
		else if (sprite->animation() != MOVE_LEFT && sprite->animFinished())
	...
	{
		if (direction != "right" && sprite->animFinished()){
			sprite->changeAnimation(STANDLR);
			direction = "right";
		}
		else if (sprite->animation() != MOVE_RIGHT && sprite->animFinished())
